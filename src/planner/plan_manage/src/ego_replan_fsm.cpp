...
void EGOReplanFSM::init(ros::NodeHandle &nh) {
  ...
  safety_timer_ = nh.createTimer(ros::Duration(0.01),
                                 &EGOReplanFSM::checkCollisionCallback, this);

  odom_sub_ =
      nh.subscribe("/odom_world", 1, &EGOReplanFSM::odometryCallback, this);
  cmd_sub_ = nh.subscribe("/planning/cmd", 1, &EGOReplanFSM::cmdCallback, this);
  setpoint_sub_ = nh.subscribe("/fast_goal", 1, &EGOReplanFSM::setpointCallback,
                               this, ros::TransportHints().tcpNoDelay());

  bspline_pub_ = nh.advertise<ego_planner::Bspline>("/planning/bspline", 10);
  data_disp_pub_ =
      nh.advertise<ego_planner::DataDisp>("/planning/data_display", 100);
  ...
  if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    ...
    while (ros::ok() && !have_odom_) ros::spinOnce();
...
void EGOReplanFSM::cmdCallback(const controller_msgs::cmdConstPtr &msg) {
  switch (msg->cmd) {
    ...
    case (1):  // stop
      ...
      if (exec_state_ != WAIT_TARGET) changeFSMExecState(WAIT_TARGET, "FSM");
      break;
    case (10):  // armming
    case (11):  // diarmming
    case (20):  // takeoff
...
void EGOReplanFSM::setpointCallback(const geometry_msgs::PoseStamped &msg) {
  setpoint_pos_[0] = msg.pose.position.x;
  setpoint_pos_[1] = msg.pose.position.y;
  setpoint_pos_[2] = msg.pose.position.z;

  setpoint_orient_.w() = msg.pose.orientation.w;
  setpoint_orient_.x() = msg.pose.orientation.x;
  setpoint_orient_.y() = msg.pose.orientation.y;
  setpoint_orient_.z() = msg.pose.orientation.z;
...
void EGOReplanFSM::planGlobalTrajbyGivenWps() {
  ...
  for (size_t i = 0; i < (size_t)waypoint_num_; i++) {
    visualization_->displayGoalPoint(wps[i], Eigen::Vector4d(0, 0.5, 0.5, 1),
                                     0.3, i);
  ...
  if (success) {
    ...
    int i_end =
        floor(planner_manager_->global_data_.global_duration_ / step_size_t);
    std::vector<Eigen::Vector3d> gloabl_traj(i_end);
    for (int i = 0; i < i_end; i++) {
      gloabl_traj[i] =
          planner_manager_->global_data_.global_traj_.evaluate(i * step_size_t);
    }
    ...
    // visualization_->displayGoalPoint(end_pt_, Eigen::Vector4d(1, 0, 0, 1),
    // 0.3, 0);
    ros::Duration(0.001).sleep();
...
void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg) {
  ...
  end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y,
      std::max(1.5, msg->poses[0].pose.position.z);
...
void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state,
  ...
  static string state_str[7] = {"INIT",        "WAIT_TARGET", "GEN_NEW_TRAJ",
...
void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e) {
  ...
  switch (exec_state_) {
    ...
    case GEN_NEW_TRAJ: {
      ...
      // Eigen::Vector3d rot_x = odom_orient_.toRotationMatrix().block(0, 0, 3,
      // 1); start_yaw_(0)         = atan2(rot_x(1), rot_x(0)); start_yaw_(1) =
      // start_yaw_(2) = 0.0;
    ...
    case EMERGENCY_STOP: {
      if (flag_escape_emergency_)  // Avoiding repeated calls
      {
        ...
        if (odom_vel_.norm() < 0.1) changeFSMExecState(GEN_NEW_TRAJ, "FSM");
      }
...
bool EGOReplanFSM::planFromCurrentPos() {
  LocalTrajData *info = &planner_manager_->local_data_;
  ros::Time time_now = ros::Time::now();
  double t_cur = (time_now - info->start_time_).toSec();

  // cout << "info->velocity_traj_=" <<
  // info->velocity_traj_.get_control_points() << endl;
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e) {
  ...
  if (exec_state_ == WAIT_TARGET || info->start_time_.toSec() < 1e-5) return;
  ...
  for (double t = t_cur; t < info->duration_; t += time_step) {
    if (t_cur < t_2_3 &&
        t >= t_2_3)  // If t_cur < t_2_3, only the first 2/3 partition of the
    ...
    if (map->getInflateOccupancy(info->position_traj_.evaluateDeBoorT(t))) {
      if (planFromCurrentTraj())  // Make a chance
      {
        ...
        {
          ROS_WARN("Suddenly discovered obstacles. emergency stop! time=%f",
                   t - t_cur);
...
void EGOReplanFSM::getLocalTarget() {
  ...
  if ((end_pt_ - local_target_pt_).norm() <
    ...
    // local_target_vel_ = (end_pt_ - init_pt_).normalized() *
    // planner_manager_->pp_.max_vel_ * (( end_pt_ - local_target_pt_ ).norm() /
